    ...
    <section className="preview-area">
      ...
      <article className="panel" ref={panelRef} style={{background:backgroundImage?`url(${backgroundImage})`:backgroundCss,backgroundSize:"cover",backgroundPosition:`${backgroundX}% ${backgroundY}%`}}>
        ...
        <div className="panel-grid">{plays.map((p,i)=><section className={`play-card card-${i} ${p.isBookPlay?"has-book-card":""}`} key={p.id}>
          ...
          <div className={`play-copy ${p.isBookPlay?"book-copy":""}`}><h4 className="preview-editable" contentEditable={!p.approved&&!!p.title} suppressContentEditableWarning onInput={e=>updatePlay(i,{title:e.currentTarget.textContent||"",approved:false})}>{p.approved?p.publishedTitle:(p.title||p.publishedTitle)}</h4>{p.isBookPlay?<div className="book-copy-body"><div className="book-cover-slot">{p.bookCover?<img src={p.bookCover.src} alt={`${p.approved?p.publishedTitle:p.title} 그림책 표지`} style={{objectPosition:`${p.bookCover.x}% ${p.bookCover.y}%`}}/>:<span>그림책<br/>표지</span>}</div><p className="preview-editable" contentEditable={!p.approved&&!!p.description} suppressContentEditableWarning onInput={e=>updatePlay(i,{description:e.currentTarget.textContent||"",approved:false})}>{p.approved?p.publishedDescription:(p.description||p.publishedDescription)}</p></div>:<p className="preview-editable" contentEditable={!p.approved&&!!p.description} suppressContentEditableWarning onInput={e=>updatePlay(i,{description:e.currentTarget.textContent||"",approved:false})}>{p.approved?p.publishedDescription:(p.description||p.publishedDescription)}</p>}</div>
          {!p.approved&&p.title&&p.description&&<button className="preview-confirm no-print" onClick={()=>publishDraft(i,p.title,p.description)}>확인</button>}
        </section>)}</div>
