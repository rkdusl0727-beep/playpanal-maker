...
const naturalizeNote = (note: string, playTitle: string) => {
  ...
  const learningRules: Array<[RegExp, string]> = [
    [/우드락|판화|새기|찍어내|롤러/, "선을 새기고 물감을 묻혀 찍어내는 과정을 반복하며 판화의 원리와 재료에 따른 표현의 차이를 경험했답니다."],
    [/(책|그림책|동화).*(소리|말|의성어|의태어)|(소리|말|의성어|의태어).*(책|그림책|동화)/, "그림책 속 말과 소리의 느낌을 살펴보고 자신이 떠올린 장면을 새로운 방식으로 표현해보았어요."],
    [/(책|그림책|동화)/, "이야기의 장면을 자신의 경험과 연결하고 기억에 남은 생각과 느낌을 다양한 방법으로 표현해보았어요."],
    [/(소리|노래|악기|리듬|천둥|귀뚜라미)/, "주변에서 들리는 소리의 특징을 비교하고 몸짓과 말, 도구로 표현하는 즐거움을 느꼈어요."],
    [/(비|빗방울|물웅덩이).*(번지|수채|물감)|(번지|수채).*(비|빗방울)/, "빗방울의 모습과 물감이 번지는 변화를 관찰하며 날씨가 주는 느낌을 색으로 표현해보았어요."],
    [/(분필).*(옥상|정원|바닥)|(옥상|정원|바닥).*(분필)/, "넓은 바닥을 함께 사용하는 방법을 찾아가며 색과 선으로 떠오른 장면을 자유롭게 펼쳐보았어요."],
    [/(물놀이|물총|물길|얼음|거품|파도)/, "물의 흐름과 움직임을 몸으로 느끼고 도구에 따라 달라지는 변화를 탐색하며 즐겁게 놀이했어요."],
    [/(꽃|나뭇잎|나무|숲|곤충|흙|돌|열매|자연)/, "자연물의 모양과 색, 촉감을 자세히 살펴보고 발견한 특징을 놀이와 표현으로 이어갔답니다."],
    [/(블록|쌓기|구성|길 만들|집 만들)/, "공간과 블록의 관계를 살피며 생각한 구조를 구성하고, 친구와 의견을 조율해 놀이 공간을 넓혀보았어요."],
    [/(역할|가게|병원|가족|시장|소꿉|극놀이)/, "경험한 장면을 역할과 말로 재구성하고 서로의 생각을 주고받으며 이야기를 함께 만들어보았어요."],
    [/(달리기|점프|균형|공|신체|몸짓|춤|체조)/, "몸의 움직임과 공간의 변화를 느끼며 움직임을 조절하고 친구와 함께하는 즐거움을 경험했어요."],
    [/(요리|반죽|썰기|섞기|간식|음식)/, "재료의 모양과 냄새, 질감이 달라지는 과정을 살펴보고 순서와 약속을 지키며 함께 완성했답니다."],
    [/(실험|자석|빛|그림자|관찰|측정|돋보기)/, "궁금한 점을 직접 시험하고 변화를 자세히 관찰하며 자신의 생각을 말로 설명했어요."],
    [/(사진|카메라|만화|장면|순서)/, "관찰한 모습을 사진과 장면으로 담고 순서를 연결하며 자신만의 이야기를 구성했답니다."],
    [/(그림|색|물감|미술|꾸미|그리|만들기)/, "재료의 색과 질감, 표현 방법의 차이를 탐색하며 떠오른 생각과 느낌을 나타내는 즐거움을 느꼈어요."],
  ];
  const learning = learningRules.find(([pattern]) => pattern.test(context))?.[1]
    || "놀이에 필요한 재료와 방법을 스스로 선택하고 친구와 생각을 나누며 경험을 자연스럽게 넓혀보았어요.";
  const varied = [...details, learning].map((sentence, index) => {
  ...
  return <main className="app-shell">
    <aside className="editor no-print">
      ...
      {plays.map((p, pi) => <section className="play-editor" id={`edit-${p.id}`} key={p.id}>
        ...
        {(p.title || p.description) && <div className="draft-result"><div className="draft-result-head"><b>생성된 놀이신문</b><span>위에서 다듬은 내용이 그대로 신문에 반영됩니다</span></div><label>놀이 제목<input value={p.title} onChange={e=>updatePlay(pi,{title:e.target.value,approved:false})}/></label><label>놀이 설명 <span className="description-guide">실제 놀이신문과 같은 글꼴·크기·줄 간격 ({p.description.trim().length}자)</span><div className={`newspaper-description ${p.isBookPlay?"book-description":""} ${pi===4?"wide-description":""}`} contentEditable suppressContentEditableWarning role="textbox" aria-multiline="true" onInput={e=>updatePlay(pi,{description:e.currentTarget.innerText,approved:false})}>{p.description}</div></label><button className={p.approved?"approved":"approve"} disabled={p.approved} onClick={()=>publishDraft(pi,p.title,p.description)}>{p.approved?"✓ 반영 완료":"확인"}</button></div>}
