...
const naturalizeNote = (note: string, playTitle: string) => {
  ...
  if (/(블록|놀잇감)/.test(context) && /(여름소리|여름 소리)/.test(context)) {
    return /확장활동/.test(context)
      ? "어제 관심을 보였던 여름 소리를 다시 들어보고, 교실의 블록과 악기, 색연필 등 다양한 놀잇감으로 소리를 만들어보았어요. 친구와 두 명씩 짝을 지어 파도, 천둥, 매미, 빗소리와 빗방울 소리를 표현하는 놀이로 자연스럽게 확장되고 있답니다."
      : "어제 관심을 보였던 여름 소리를 다시 들어보고, 교실의 블록과 악기, 색연필 등 다양한 놀잇감으로 소리를 만들어보았어요. 친구와 두 명씩 짝을 지어 파도, 천둥, 매미, 빗소리와 빗방울 소리를 표현하며 느낀 점을 자연스럽게 나누어보았어요.";
  }
